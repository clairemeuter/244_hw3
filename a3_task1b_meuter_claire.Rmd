...
This data set and additional background information can be accessed at: https://www.davidzeleny.net/anadat-r/doku.php/en:data:grasslands


Reference: Zelený D. & Li Ch.-F. (2008): Vegetace suchých acidofilních trávníků v oblasti třebíčského žulosyenitového plutonu [Dry acidophilous grassland vegetation in the area of Třebíč's granosyenite pluton] - Acta Rerum Naturalium, 5: 177-194 pdf
...
veg <- read_excel(here("data","grasslands-spe-env.xls"), sheet = 2) %>% # this excel file had several pages including metadata. I read in sheet 2, which has the variables I'm looking to explore
  ...
  dplyr::rename(cover_shrub = coverE2, cover_herb = coverE1) # I am jsut renaming these columns so they're easier to understand later 
...
Autoplot is another handy option for looking at biplots. 
```{r}
veg_autoplot <- autoplot(veg_pca, 
  ...
  theme_minimal()
```
## To further explore my data, I can create a screeplot to see how the variance is captured by my principle compenents 
```{r}
